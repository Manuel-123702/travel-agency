...
export default function AdminOverview() {
  ...
  const pendingTasks = [
    {
      type: "document",
      text: "Review bank statement — Marcus Johnson",
      time: "Due today",
      urgent: true,
    },
    {
      type: "message",
      text: "Reply to Fatima Al-Rashidi",
      time: "48h ago",
      urgent: true,
    },
    {
      type: "document",
      text: "Approve police clearance — Yves Dupont",
      time: "Due tomorrow",
      urgent: false,
    },
    {
      type: "client",
      text: "Assign advisor to Priya Nair",
      time: "New intake",
      urgent: true,
    },
    {
      type: "document",
      text: "Send evaluation report — Omar Hassan",
      time: "2 days",
      urgent: false,
    },
  ];
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-3">
          <div className="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
            ...
            <p className="mt-2 text-sm text-gray-500">
              Redis, database, and Sentry status in real time.
            </p>
          </div>
          <div className="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
            <p className="text-xs uppercase tracking-[0.24em] text-gray-500">
            ...
            <p className="mt-3 text-xl font-semibold text-gray-900">
              {health ? (health.redis.healthy ? "Healthy" : "Down") : "—"}
            </p>
            <p className="mt-2 text-sm text-gray-500">
              Enabled: {health?.redis.enabled ? "Yes" : "No"}
            </p>
          </div>
          <div className="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
            <p className="text-xs uppercase tracking-[0.24em] text-gray-500">
            ...
            <p className="mt-3 text-xl font-semibold text-gray-900">
              {health ? (health.database.healthy ? "Healthy" : "Issue") : "—"}
            </p>
            <p className="mt-2 text-sm text-gray-500">
              {health?.database.error ?? "No issues"}
            </p>
          </div>
        </div>
      </motion.div>
        ...
        {kpis.map(({ icon: Icon, label, value, change, color, up }, i) => (
          ...
          >
            <div className="flex items-center justify-between mb-3">
              ...
              <span
                className={`text-xs font-semibold flex items-center gap-0.5 ${up ? "text-green-600" : "text-blue-600"}`}
              >
                <ArrowUpRight size={12} className={up ? "" : "rotate-90"} />
                {change}
              </span>
            </div>
            <p className="font-heading font-black text-gray-900 text-3xl">
              {value}
            </p>
      ...
      <div className="grid lg:grid-cols-3 gap-6 mb-6">
        ...
        >
          <div className="flex items-center justify-between p-5 border-b border-gray-100">
            ...
            <Link
              href="/admin/clients"
              className="text-blue-700 text-sm font-semibold hover:underline flex items-center gap-1"
            >
            ...
            {pendingTasks.map(({ type, text, time, urgent }) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p
                    className={`text-sm leading-tight ${urgent ? "text-red-800 font-medium" : "text-gray-700"}`}
                  >
                    {text}
                  </p>
                  <p
                    className={`text-xs mt-0.5 ${urgent ? "text-red-500 font-semibold" : "text-gray-400"}`}
                  >
                    {time}
                  </p>
                </div>
      ...
      >
        ...
        <div className="grid sm:grid-cols-3 gap-4">
          {destinations.map(({ flag, country, active, approved, rate }) => (
            <div
              key={country}
              className="p-5 bg-gray-50 rounded-2xl hover:bg-gray-100 transition-colors"
            >
