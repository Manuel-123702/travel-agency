...
import {
  ...
  Globe, Calendar, Briefcase,
} from "lucide-react";

const navItems = [
  { label: "Overview", href: "/staff", icon: LayoutDashboard, exact: true },
  { label: "My Clients", href: "/staff/clients", icon: Users },
  { label: "Messages", href: "/staff/messages", icon: MessageSquare, badge: 3 },
  { label: "Document Review", href: "/staff/documents", icon: FileText, badge: 2 },
...
export default function StaffLayout({ children }: { children: React.ReactNode }) {
  ...
  const Sidebar = () => (
    <aside className="w-64 bg-[#1E293B] h-full flex flex-col border-r border-white/5">
      {/* Logo */}
        ...
        <Link href="/" className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="flex items-center gap-1.5 mt-0.5">
              ...
              <p className="text-blue-400 text-[10px] font-bold uppercase tracking-wider">Staff Portal</p>
            </div>
          </div>
        </Link>
      </div>
  ...
  return (
    <div className="min-h-screen flex bg-[#F0F2F7]">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <div className="lg:hidden flex items-center gap-3 px-4 py-3 bg-[#1E293B] border-b border-white/8 sticky top-0 z-30">
          <button onClick={() => setMobileOpen(true)} className="p-2 text-white/60 hover:text-white transition-colors">
            <Menu size={20} />
          </button>
          <span className="font-heading font-bold text-white text-sm">
            TRAVEL <span className="text-gold">AGENCY</span>
            <span className="text-blue-400 text-xs ml-2 font-normal">Staff</span>
          </span>
